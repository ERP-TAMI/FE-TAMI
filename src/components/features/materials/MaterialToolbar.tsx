...
type Props = {
  ...
  onMaterialGroupChange: (value: string) => void;
    ...
    <header className="border-b border-gray-200 bg-gray-50 px-4 py-5 sm:px-6 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="flex flex-1 flex-wrap items-start gap-2.5">
        ...
        <div
          ...
          className="flex h-11 items-center rounded-lg border border-gray-200 bg-white p-0.5 dark:border-gray-700 dark:bg-gray-900"
        >
          {statusOptions.map((option) => (
            <button
              ...
              className={`text-theme-xs rounded-md px-3 py-1.5 font-medium transition-colors ${status === option.key ? "bg-brand-50 text-brand-600 dark:bg-brand-500/15 dark:text-brand-400" : "text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"}`}
            >
              {option.label}
            </button>
          ))}
