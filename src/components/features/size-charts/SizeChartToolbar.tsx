...
import type { SizeChartStatus } from "@/types/size-chart";

type SizeChartToolbarProps = {
  search: string;
  status: SizeChartStatus | "";
  onSearchChange: (value: string) => void;
  onStatusChange: (value: SizeChartStatus | "") => void;
};

const statusOptions: Array<{ key: SizeChartStatus | ""; label: string }> = [
  { key: "", label: "Tất cả" },
  { key: "active", label: "Đang sử dụng" },
  { key: "inactive", label: "Đã tắt" },
];

export function SizeChartToolbar({
  search,
  status,
  onSearchChange,
  onStatusChange,
}: SizeChartToolbarProps) {
  return (
    <header className="border-b border-gray-200 bg-gray-50 px-4 py-5 sm:px-6 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="flex flex-1 flex-wrap items-center gap-2.5">
        <div className="relative max-w-md min-w-[220px] flex-1">
          ...
          <Input
            type="search"
            aria-label="Tìm kiếm bảng Size"
            placeholder="Tìm theo tên bảng hoặc Size..."
            value={search}
