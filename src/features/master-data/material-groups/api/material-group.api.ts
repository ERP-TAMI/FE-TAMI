...
import { materialGroupListSchema, materialGroupResponseSchema } from "../schemas/material-group.schema";
...
export const materialGroupApi = {
  async list(status?: MaterialGroupStatus): Promise<MaterialGroup[]> {
    const response = await apiClient.get<MaterialGroup[]>(resource, { params: status ? { status } : undefined });
