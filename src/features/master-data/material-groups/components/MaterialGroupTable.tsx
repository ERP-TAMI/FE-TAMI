...
export function MaterialGroupTable({ materialGroups, onEdit, onStatus, onDelete }: MaterialGroupTableProps) {
  return (
    <Table
      rows={materialGroups}
      ...
      columns={[
        { key: "code", header: "Code", render: (group) => <span className="font-medium">{group.code}</span> },
        { key: "name", header: "Name", render: (group) => group.name },
        ...
        },
        {
          ...
          render: (group) => (
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={() => onEdit(group)}>Edit</Button>
