...
import type { MaterialGroup, MaterialGroupInput, MaterialGroupStatus } from "../types/material-group.types";
...
export default function MaterialGroupListPage() {
  ...
  const mutation = create.isPending || update.isPending || updateStatus.isPending || remove.isPending;
  ...
  const confirmDialog = async () => {
    ...
    try {
      if (dialog.type === "delete") {
        ...
        const nextStatus: MaterialGroupStatus = dialog.materialGroup.status === "active" ? "inactive" : "active";
    ...
    <>
      ...
      <section aria-labelledby="page-title" className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p className="text-theme-xs text-brand-500 font-medium tracking-wider uppercase">Master data</p>
            <h1 id="page-title" className="text-title-md mt-2 font-semibold text-gray-900 dark:text-white">Material groups</h1>
            <p className="text-theme-sm mt-2 text-gray-500 dark:text-gray-400">Maintain the groups used to organize materials.</p>
          </div>
